/*
 * (#) net.brainage.nest.service.security.NestUserDetailsService.java
...
public class SigninUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String usernameOrEmail) throws UsernameNotFoundException {
        ...
        if (user == null && usernameOrEmail.indexOf('@') > 0) {
            user = userRepository.findByEmail(usernameOrEmail);
        }
        if (user == null) {
            throw new UsernameNotFoundException(
                    String.format("The username or email for '%s' is not found.", usernameOrEmail));
        }
